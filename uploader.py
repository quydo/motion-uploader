...
class MotionUploader:
    def __init__(self, config_file_path, account='account1'):
        # Load config
    ...
    def _create_drive(self):
        ...
        storage = Storage(self.oauth_folder+self.account+'.txt')
        ...
        if auth_required:
            flow = client.flow_from_clientsecrets(
                self.oauth_folder+self.account+'.json',
    ...
    def _get_folder_id(self, folder_name):
        ...
        if len(files['items']) == 1:
            ...
            print 'no folder {}, creating it'.format(folder_name)
            file_metadata = {
                'title' : folder_name,
    ...
    def upload_video(self, video_file_path):
        ...
        media = MediaFileUpload(video_file_path, mimetype='video/avi', resumable=True)

        request = self.drive_service.files().insert(media_body=media, body={'title':os.path.basename(video_file_path), 'parents':[{u'id': folder_id}]})
